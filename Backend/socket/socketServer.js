...
async function _authMiddleware(socket, next) {
  try {
    ...
    // Normalise userType – the existing JWT may use 'role' instead of 'userType'
    const userType = decoded.userType || decoded.role;
    if (!userType || !['doctor', 'patient'].includes(userType.toLowerCase())) {
    ...
    socket.data.userType = userType.toLowerCase();
    socket.data.userName = decoded.name || decoded.userName || 'Unknown';
