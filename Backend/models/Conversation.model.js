...
conversationSchema.pre('save', async function(next) {
  if (this.isNew) {
    try {
      ...
      if (!doctor) {
        throw new Error('Invalid doctor ID: Doctor does not exist');
      }
      ...
      if (!patient) {
        throw new Error('Invalid patient ID: Patient does not exist');
      }

      // Validate doctor-patient assignment relationship
      // Patient's docID should match the doctor's _id
      if (!patient.docID || patient.docID.toString() !== this.doctorId.toString()) {
        throw new Error('No active doctor-patient assignment exists');
      }
