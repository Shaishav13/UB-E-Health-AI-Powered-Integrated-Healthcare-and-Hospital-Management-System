...
import { handleChatApiError } from "../../utils/chatApiError";
...
export const fetchConversations =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      const { message: errorMessage } = handleChatApiError(error, { context: 'fetchConversations' });
...
export const fetchConversationMessages =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      const { message: errorMessage } = handleChatApiError(error, { context: 'fetchConversationMessages' });
 ...
 * Supports cursor-based pagination when `cursor` is provided (Req 20.4).
 */
export const loadMoreMessages =
  (conversationId, page, cursor = null) => async (dispatch) => {
    ...
    try {
      ...
      const { message: errorMessage } = handleChatApiError(error, { context: 'loadMoreMessages' });
...
/**
 * Prefetch the next page of messages when user scrolls to 80% (Req 13.2, 20.4).
 * Results are stored in Redux but not yet displayed — they are merged when
 * the user actually scrolls to the top.
 */
export const prefetchMessages =
  (conversationId, cursor) => async (dispatch) => {
    if (!cursor) return;
    dispatch({ type: types.PREFETCH_MESSAGES_REQUEST, payload: { conversationId } });
    try {
      const res = await axios.get(
        `${API_URL}/api/chat/conversations/${conversationId}`,
        {
      ...
      dispatch({
        type: types.PREFETCH_MESSAGES_SUCCESS,
        payload: {
          conversationId,
          messages: res.data.messages,
          pagination: res.data.pagination,
      ...
      dispatch({
        type: types.PREFETCH_MESSAGES_ERROR,
        payload: { conversationId, message: error.message },
      });
      return { error: true };
    }
  };
...
export const createConversation =
  (doctorId, patientId) => async (dispatch) => {
    ...
    try {
      ...
      const { message: errorMessage } = handleChatApiError(error, { context: 'createConversation' });
...
export const archiveConversation = (conversationId) => async (dispatch) => {
  ...
  try {
    ...
    const { message: errorMessage } = handleChatApiError(error, { context: 'archiveConversation' });
...
export const unarchiveConversation = (conversationId) => async (dispatch) => {
  ...
  try {
    ...
    const { message: errorMessage } = handleChatApiError(error, { context: 'unarchiveConversation' });
...
export const sendMessage = (socket, payload) => (dispatch) => {
  ...
  if (socket && socket.connected) {
    ...
    chatToast.warn("Not connected to chat server. Your message will be sent when reconnected.");
    dispatch({
...
export const markMessageAsRead = (messageId, conversationId) => async (dispatch) => {
  ...
  try {
    ...
    const { message: errorMessage } = handleChatApiError(error, { context: 'markMessageAsRead', silent: true });
...
export const deleteMessage = (messageId, conversationId) => async (dispatch) => {
  ...
  try {
    ...
    const { message: errorMessage } = handleChatApiError(error, { context: 'deleteMessage' });
...
export const fetchUnreadCount = () => async (dispatch) => {
  ...
  try {
    ...
    const { message: errorMessage } = handleChatApiError(error, { context: 'fetchUnreadCount', silent: true });
...
export const searchMessages = (conversationId, query) => async (dispatch) => {
  ...
  try {
    ...
    const { message: errorMessage } = handleChatApiError(error, { context: 'searchMessages' });
