...
router.get("/patients", authenticate, async (req, res) => {
  try {
    const doctorID = req.body.doctorID; // numeric doctorId string from JWT (e.g. "1")
    const mongoose = require("mongoose");

    // Step 1: Find the doctor's ObjectId from their numeric doctorId
    ...
    // Step 2: Find patients via two sources and merge (deduplicated):
    //   a) Patients whose docID matches this doctor's numeric doctorId (assignment field)
    //   b) Patients who have at least one report created by this doctor
    //      (covers reports created before the auto-assignment fix)

    const Patient = mongoose.model("Patient");
    const Report = mongoose.model("Report");

    // a) Patients assigned via docID
    const assignedPatients = await Patient.find({ docID: Number(doctorID) }).lean();

    // b) Patients found via reports
    const reports = await Report.find({ doctorid: doctorObjectId })
      .distinct("patientid");

    const reportPatients = reports.length > 0
      ? await Patient.find({ _id: { $in: reports } }).lean()
      : [];
    ...
    // Step 3: Also backfill docID for any patients found via reports but not yet assigned
    const unassigned = reportPatients.filter((p) => !p.docID || Number(p.docID) !== Number(doctorID));
    if (unassigned.length > 0) {
      await Patient.updateMany(
        { _id: { $in: unassigned.map((p) => p._id) } },
        { $set: { docID: Number(doctorID) } }
      );
      console.log(`✅ Backfilled docID for ${unassigned.length} patient(s) of doctor ${doctor.name}`);
    }

    res.status(200).send(allPatients);
  } catch (error) {
    console.error("GET /doctors/patients error:", error);
    res.status(400).send({ error: "Something went wrong", message: error.message });
  }
});
