...
/**
 * Current connection status exposed to the rest of the app.
 * 'idle' | 'connecting' | 'connected' | 'disconnected' | 'failed'
 * @type {string}
 */
let _connectionStatus = "idle";
...
/**
 * Update the internal connection status and notify all listeners.
 * @param {string} status - 'idle'|'connecting'|'connected'|'disconnected'|'failed'
 * @param {number} [attempt] - reconnect attempt number (for 'connecting')
 */
function setConnectionStatus(status, attempt) {
  _connectionStatus = status;
  _statusListeners.forEach((fn) => fn(status, attempt));
  // Show user-facing toast for meaningful state changes (Req 23.1, 23.8)
  chatToast.connectionStatus(status, attempt);
}
...
function registerEventListeners() {
  ...
  socket.on("message_error", ({ tempId, error, code }) => {
    ...
    // Show a user-friendly toast for common error codes
    if (code === 429) {
      chatToast.rateLimitWarning(60); // default 60s if no reset time provided
    } else if (code === 403) {
      chatToast.error("You don't have permission to send messages in this conversation.");
    } else if (error) {
      chatToast.error(error);
    }
  });
}
...
function scheduleReconnect() {
  ...
  // Notify UI that we are about to reconnect (Req 23.1)
  setConnectionStatus("connecting", _reconnectAttempts);
 ...
 * @returns {'idle'|'connecting'|'connected'|'disconnected'|'failed'}
 */
 ...
 * @param {(status: string, attempt?: number) => void} listener
